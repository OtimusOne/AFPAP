...
with open(f'./output/work/molecular_docking_{args.name}_mqc.txt', 'w') as f:
    with open('config/molecularDocking_template.txt', 'r') as template:
        ...
        for i, row in df.iterrows():
            bestScore, meanScore, stdScore = dock(receptor=args.receptor,ligand=args.ligand,center=[row['Center X'],row['Center Y'],row['Center Z']],box_size=[20,20,20],spacing=0.375)
            print(row['Name'],bestScore,f"{meanScore} \u00B1 {stdScore}",f"{np.round(row['Center X'],2)},{np.round(row['Center Y'],2)},{np.round(row['Center Z'],2)}", "20,20,20",0.375,32,sep='\t',file=f)
        center_x,box_x,center_y,box_y,center_z,box_z=wideBox(args.receptor)
        bestScore, meanScore, stdScore = dock(receptor=args.receptor,ligand=args.ligand,center=[center_x,center_y,center_z], box_size=[box_x,box_y,box_z],spacing=1)
        print('Wide Box',bestScore,f"{meanScore} \u00B1 {stdScore}",f"{np.round(center_x,2)},{np.round(center_y,2)},{np.round(center_z,2)}",f"{int(box_x)},{int(box_y)},{int(box_z)}",1,32,sep='\t',file=f)
